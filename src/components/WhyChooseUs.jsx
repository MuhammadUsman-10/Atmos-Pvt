import React, {useState, useEffect, useRef} from 'react';

const WhyChooseUs = () => {
    const scrollContainerRef = useRef(null);
    const [canScrollLeft, setCanScrollLeft] = useState(false);
    const [canScrollRight, setCanScrollRight] = useState(true);

    const checkScrollability = () => {
        const container = scrollContainerRef.current;
        if (container) {
            const { scrollLeft, scrollWidth, clientWidth } = container;
            // A small buffer helps with floating point inaccuracies
            setCanScrollLeft(scrollLeft > 10);
            setCanScrollRight(scrollLeft < scrollWidth - clientWidth - 10);
        }
    };

    useEffect(() => {
        const container = scrollContainerRef.current;
        // Run check on mount and on resize
        checkScrollability();
        window.addEventListener('resize', checkScrollability);
        
        if (container) {
            container.addEventListener('scroll', checkScrollability);
        }

        return () => {
            window.removeEventListener('resize', checkScrollability);
            if (container) {
                container.removeEventListener('scroll', checkScrollability);
            }
        };
    }, []);

    const scroll = (direction) => {
        const container = scrollContainerRef.current;
        if (container) {
            // Scroll by 80% of the visible width for a smoother experience
            const scrollAmount = container.clientWidth * 0.8;
            container.scrollBy({ left: direction * scrollAmount, behavior: 'smooth' });
        }
    };

  return (
    <div id="whychooseus" className="relative py-10 bg-[#222429]">
      <div className='relative container mx-auto px-12 xxs:px-14 lg:px-16 xl:px-18'>
        <h2 className="text-4xl font-bold text-center uppercase text-white mb-4">
          Why <span className="text-[#ffaa17]">Choose</span> Us
        </h2>
        <p className="text-gray-300 text-lg lg:text-2xl text-center my-12">
          Why Security Companies Trust ATMOS.
        </p>
        <div className="relative">
            <div ref={scrollContainerRef} className="flex overflow-x-auto snap-x snap-mandatory scroll-smooth scrollbar-hide gap-8">
              {/* Card 1 */}
              <div className="group relative flex flex-col items-left gap-8 text-left p-6 lg:p-10 bg-[#1f1f1f] rounded-lg shadow-lg w-[240px] xxs:w-[260px] xs:w-[280px] xxsm:w-[330px] xsm:w-[250px] sm:w-[245px] md:w-[310px] lg:w-[300px] flex-shrink-0 overflow-hidden snap-start">
                <div className="absolute inset-0 bg-[#ffaa17] transform origin-bottom scale-y-0 group-hover:scale-y-100 transition-transform duration-700 ease-in-out"></div>
                <div className="relative z-10">
                  <svg fill="currentColor" className="h-16 w-16 group-hover:text-black text-[#ffaa17] mb-6"  version="1.0" xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 512.000000 512.000000"
                      preserveAspectRatio="xMidYMid meet">
                      <g transform="translate(0.000000,512.000000) scale(0.100000,-0.100000)"
                      stroke="none">
                      <path d="M1135 4790 c-205 -33 -384 -126 -535 -279 -129 -130 -213 -279 -257 -457 -27 -110 -24 -335 5 -444 50 -182 122 -309 247 -436 100 -101 145 -134 261 -192 180 -91 274 -104 726 -100 l346 3 -55 89 -55 89 119 119 120 120 69 -43 c38 -23 79 -49 92 -57 l24 -15 -4 404 c-4 382 -6 408 -27 484 -53 186 -122 305 -256 440 -135 135 -257 206 -440 255 -92 25 -284 35 -380 20z m-15 -374 l0 -145 117 40 c64 21 118 39 120 39 2 0 3 -36 3 -79 l0 -79 80 26 80 26 0 158 0 158 80 0 80 0 0 -130 c0 -71 1 -130 3 -130 2 0 37 11 77 25 40 14 75 25 77 25 2 0 3 -187 3 -415 l0 -415 40 0 40 0 0 -80 0 -80 -640 0 -640 0 0 80 0 80 40 0 40 0 0 309 0 309 118 39 117 40 3 171 2 172 80 0 80 0 0 -144z"/>
                      <path d="M1040 4076 l-155 -53 -3 -251 -2 -252 160 0 160 0 0 305 c0 168 -1 305 -2 304 -2 0 -73 -24 -158 -53z"/>
                      <path d="M1520 4076 l-155 -53 -3 -251 -2 -252 160 0 160 0 0 305 c0 168 -1 305 -2 304 -2 0 -73 -24 -158 -53z"/>
                      <path d="M3690 4789 c-135 -24 -286 -85 -397 -160 -26 -18 -88 -73 -138 -124 -124 -126 -195 -251 -246 -430 -21 -76 -23 -102 -27 -484 -3 -333 -1 -402 10 -395 7 4 49 30 92 57 l79 49 119 -120 120 -119 -49 -79 c-27 -43 -53 -85 -57 -92 -7 -11 62 -13 395 -10 373 4 410 6 480 26 187 51 341 142 466 276 116 124 188 256 235 426 32 118 32 336 0 457 -47 180 -126 320 -256 449 -128 128 -270 210 -445 255 -89 23 -297 33 -381 18z m624 -401 c55 -16 138 -99 154 -154 17 -58 17 -730 0 -788 -16 -55 -99 -138 -154 -154 -58 -17 -890 -17 -948 0 -55 16 -138 99 -154 154 -8 29 -12 146 -12 394 0 248 4 365 12 394 15 51 99 137 148 153 52 16 899 17 954 1z"/>
                      <path d="M3385 4215 l-25 -24 0 -351 0 -351 25 -24 24 -25 431 0 431 0 24 25 25 24 0 351 0 351 -25 24 c-23 24 -29 25 -160 25 l-135 0 0 -240 0 -240 -240 0 -240 0 0 240 0 240 -55 0 c-46 0 -60 -4 -80 -25z"/>
                      <path d="M3680 4080 l0 -160 80 0 80 0 0 160 0 160 -80 0 -80 0 0 -160z"/>
                      <path d="M2517 3273 c-2 -4 -12 -45 -22 -90 l-18 -82 -120 -49 -119 -50 -78 48 -77 48 -31 -30 -30 -31 47 -77 48 -77 -49 -120 -50 -120 -86 -19 -87 -19 0 -45 0 -45 87 -19 86 -19 50 -120 49 -120 48 77 47 77 -30 31 -31 30 -77 -47 -77 -48 -120 49 -120 50 -19 86 -19 87 -42 3 c-23 2 -43 -1 -46 -5z m178 -424 c63 -30 123 -90 155 -154 20 -41 25 -65 25 -135 -1 -77 -4 -91 -34 -148 -38 -70 -70 -101 -145 -140 -43 -22 -64 -26 -136 -26 -70 -1 -94 4 -135 24 -260 128 -238 495 35 594 63 23 170 16 235 -15z"/>
                      <path d="M1137 2229 c-128 -19 -291 -83 -406 -161 -82 -55 -222 -207 -272 -293 -103 -179 -134 -295 -133 -495 0 -160 13 -229 65 -359 46 -114 114 -216 209 -312 132 -135 287 -222 471 -266 114 -27 328 -25 439 5 188 51 309 121 445 257 134 135 203 254 256 440 21 76 23 102 27 484 3 333 1 402 -10 395 -7 -4 -49 -30 -92 -57 l-79 -49 -119 119 -120 120 43 69 c23 38 48 79 57 92 l14 22 -368 -1 c-203 -1 -395 -5 -427 -10z m274 -178 c69 -33 113 -74 149 -142 28 -52 33 -75 37 -148 5 -76 2 -97 -26 -182 -17 -53 -31 -100 -31 -105 0 -6 47 -45 105 -88 120 -89 167 -139 183 -195 16 -53 16 -410 0 -465 -16 -55 -99 -138 -154 -154 -58 -17 -730 -17 -788 0 -55 16 -138 99 -154 154 -16 55 -16 412 0 465 16 56 63 106 183 195 58 43 105 82 105 88 0 5 -14 52 -31 105 -27 85 -30 106 -26 181 9 161 96 273 242 311 49 13 158 2 206 -20z"/>
                      <path d="M1215 1906 c-37 -17 -70 -52 -84 -89 -21 -54 -12 -118 36 -264 26 -78 45 -144 43 -146 -3 -2 -70 -52 -150 -112 -80 -59 -153 -116 -162 -127 -16 -18 -18 -44 -18 -210 0 -187 0 -189 25 -213 l24 -25 351 0 351 0 24 25 c25 24 25 26 25 213 0 166 -2 192 -17 210 -10 11 -83 68 -163 127 -80 60 -147 110 -150 112 -2 2 17 69 44 150 41 124 48 154 44 204 -5 62 -23 97 -70 132 -34 25 -113 32 -153 13z"/>
                      <path d="M3196 2228 c4 -7 30 -49 57 -92 l49 -79 -120 -119 -119 -120 -79 49 c-43 27 -85 53 -92 57 -11 7 -13 -62 -10 -395 4 -382 6 -408 27 -484 53 -187 122 -306 256 -440 135 -134 260 -207 445 -257 118 -32 336 -32 457 0 294 76 533 278 652 549 57 131 75 221 75 383 1 153 -16 243 -66 365 -93 223 -290 423 -507 515 -166 71 -184 73 -630 78 -333 3 -402 1 -395 -10z m976 -405 l308 -176 0 -463 0 -464 -640 0 -640 0 0 463 0 463 318 182 c174 100 323 179 331 177 8 -3 153 -85 323 -182z"/>
                      <path d="M3593 1687 l-232 -132 -1 -337 0 -338 80 0 80 0 0 320 0 320 320 0 320 0 0 -320 0 -320 80 0 80 0 0 338 0 337 -233 133 c-128 72 -239 132 -247 132 -8 0 -119 -60 -247 -133z"/>
                      <path d="M3680 1280 l0 -80 160 0 160 0 0 80 0 80 -160 0 -160 0 0 -80z"/>
                      <path d="M3680 960 l0 -80 160 0 160 0 0 80 0 80 -160 0 -160 0 0 -80z"/>
                      </g>
                  </svg>
                  <h2 className="max-w-[40%] text-xl sm:text-2xl lg:text-3xl font-bold text-white group-hover:text-black mb-4">Industry Focused</h2>
                  <p className="text-base sm:text-xl text-gray-300 group-hover:text-black">We specialize in the security sector — we understand your operations, language, and pain points.</p>
                </div>
              </div>

              {/* Card 2 */}
              <div className="group relative flex flex-col items-left gap-8 text-left p-6 lg:p-10 bg-[#1f1f1f] rounded-lg shadow-lg w-[240px] xxs:w-[260px] xs:w-[280px] xxsm:w-[330px] xsm:w-[250px] sm:w-[245px] md:w-[310px] lg:w-[300px] flex-shrink-0 overflow-hidden snap-start">
                <div className="absolute inset-0 bg-[#ffaa17] transform origin-bottom scale-y-0 group-hover:scale-y-100 transition-transform duration-700 ease-in-out"></div>
                <div className="relative z-10">
                  <svg fill="currentColor" className="h-16 w-16 group-hover:text-black text-[#ffaa17] mb-6" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" id="budget">
                      <path fillRule="evenodd" d="M373.699 376.187h115.866c5.723 0 10.434-4.711 10.434-10.435v-13.534c0-5.723-4.711-10.435-10.434-10.435H373.699c-5.723 0-10.435 4.711-10.435 10.435v13.534c0 5.724 4.712 10.435 10.435 10.435zm0-50.403h115.866c5.723 0 10.434-4.711 10.434-10.435v-13.534c0-5.724-4.711-10.435-10.434-10.435H373.699c-5.723 0-10.435 4.711-10.435 10.435v13.534c0 5.724 4.712 10.435 10.435 10.435zm0 100.806h115.866c5.723 0 10.434-4.711 10.434-10.435v-13.534c0-5.723-4.711-10.435-10.434-10.435H373.699c-5.723 0-10.435 4.711-10.435 10.435v13.534c0 5.723 4.712 10.435 10.435 10.435zm0-201.612h115.866c5.723 0 10.434-4.711 10.434-10.434V201.01c0-5.723-4.711-10.435-10.434-10.435H373.699c-5.723 0-10.435 4.712-10.435 10.435v13.534c0 5.723 4.712 10.434 10.435 10.434zm0 16c-5.723 0-10.435 4.711-10.435 10.434v13.534c0 5.723 4.711 10.435 10.435 10.435h115.866c5.723 0 10.434-4.711 10.434-10.435v-13.534c0-5.724-4.711-10.434-10.434-10.434H373.699zm-197.143-43.432c11.063 12.092 20.957 23.498 29.682 34.31-22.146 23.237-35.743 54.695-35.743 89.326 0 35.288 14.124 67.275 37.018 90.633-20.588 14.624-46.818 23.238-75.471 22.755-86.077 1.451-150.35-79.123-105.17-158.453 13.056-22.923 33.276-48.644 60.657-78.571 1.63.326 3.314.499 5.038.499h78.951c1.724 0 3.408-.173 5.038-.499zm-78.193-50.894-20.975-20.974c-7.072-7.072-7.072-18.646 0-25.719 6.015-6.016 15.538-7.032 22.696-2.438 2.659 1.707 5.713 1.979 8.63.771 2.917-1.209 4.884-3.559 5.558-6.646 1.782-8.164 9.092-14.325 17.771-14.327 8.522 0 15.955 6.007 17.772 14.327.674 3.087 2.641 5.437 5.558 6.646 2.919 1.209 5.971.936 8.629-.771 7.032-4.513 16.559-3.7 22.696 2.436v.001c7.072 7.073 7.072 18.646 0 25.719l-20.974 20.974H98.363v.001zm32.771 204.696c-5.318-.139-10.481-1.579-14.597-4.32-3.725-2.481-6.571-6.109-7.759-10.876a8.025 8.025 0 0 0-15.583 3.833c2.245 9.004 7.553 15.811 14.482 20.427 4.733 3.152 10.229 5.236 15.985 6.251v6.243a8.043 8.043 0 0 0 16.086 0v-6.09c6.995-1.066 14.121-3.619 19.716-7.865 6.725-5.102 11.197-12.408 10.964-22.213a31.267 31.267 0 0 0-.193-2.954c-1.304-10.744-7.117-17.737-14.824-22.39-6.77-4.087-14.888-6.123-22.331-7.345-5.368-.881-11.022-2.373-15.164-5.077-3.208-2.094-5.51-5.127-5.725-9.561-.233-4.798 1.861-8.669 5.162-11.353 2.586-2.102 5.884-3.562 9.459-4.283a25.869 25.869 0 0 1 4.414-.496 7.83 7.83 0 0 0 1.054-.006 25.57 25.57 0 0 1 5.913.775c5.979 1.516 11.422 5.456 14.289 12.283a8.04 8.04 0 1 0 14.829-6.221c-5.048-12.022-14.658-18.968-25.222-21.645a39.235 39.235 0 0 0-2.34-.518v-6.232a8.042 8.042 0 0 0-8.043-8.042 8.043 8.043 0 0 0-8.043 8.042v6.353c-6.094 1.228-11.816 3.801-16.425 7.548-7.084 5.76-11.575 14.121-11.069 24.548.508 10.476 5.736 17.503 13.014 22.254 6.344 4.142 14.136 6.285 21.322 7.464 5.771.948 11.932 2.445 16.614 5.271 3.744 2.261 6.557 5.564 7.157 10.515.08.66.123 1.139.13 1.446.093 3.936-1.79 6.935-4.617 9.081-3.54 2.686-8.511 4.27-13.483 4.873a37.394 37.394 0 0 1-3.538.281l-.383.007a8.048 8.048 0 0 0-1.251-.008zM92.568 182.045h78.951c5.317 0 9.697-4.379 9.697-9.696 0-5.318-4.379-9.697-9.697-9.697H92.568c-5.318 0-9.697 4.378-9.697 9.696s4.378 9.697 9.697 9.697zm207.427 25.636c-62.684 0-113.5 50.816-113.5 113.5s50.816 113.5 113.5 113.5c17.321 0 33.732-3.886 48.419-10.824a26.293 26.293 0 0 1-1.15-7.702v-12.934c-13.91 8.032-30.053 12.629-47.269 12.629-26.141 0-49.809-10.596-66.941-27.728-17.132-17.132-27.729-40.799-27.729-66.94s10.596-49.809 27.729-66.94c17.132-17.132 40.799-27.729 66.941-27.729 18.194 0 35.188 5.134 49.616 14.031a26.57 26.57 0 0 1 5.175-7.565c-3.871-3.97-6.509-9.146-7.284-14.898-14.453-6.672-30.544-10.4-47.507-10.4zm47.27 50.611v6.654c0 7.156 2.872 13.666 7.522 18.435-4.65 4.769-7.522 11.279-7.522 18.435v13.534c0 7.156 2.872 13.666 7.522 18.435-4.65 4.769-7.522 11.278-7.522 18.435v13.534c0 5.426 1.652 10.48 4.477 14.685-13.831 12.088-31.933 19.413-51.746 19.413-21.725 0-41.392-8.805-55.628-23.041-14.236-14.236-23.041-33.904-23.041-55.628 0-21.724 8.805-41.392 23.041-55.628 14.236-14.236 33.903-23.041 55.628-23.041 17.74-.002 34.109 5.871 47.269 15.778zm-14.224 33.402a8.165 8.165 0 1 1-15.059 6.317c-2.474-5.892-7.171-9.293-12.329-10.601a22.515 22.515 0 0 0-9.912-.24c-3.097.624-5.948 1.884-8.177 3.696-2.778 2.259-4.542 5.514-4.346 9.546.178 3.68 2.1 6.204 4.779 7.953 3.627 2.368 8.627 3.682 13.385 4.463 6.829 1.121 14.285 2.994 20.528 6.762 7.194 4.344 12.621 10.877 13.84 20.921.094.774.153 1.695.178 2.756.219 9.198-3.973 16.049-10.274 20.831-4.99 3.786-11.273 6.119-17.491 7.17v4.756a8.167 8.167 0 0 1-8.168 8.167 8.168 8.168 0 0 1-8.168-8.167v-4.915c-5.061-.983-9.886-2.868-14.07-5.654-6.448-4.295-11.389-10.632-13.479-19.016a8.148 8.148 0 1 1 15.825-3.893c1.017 4.081 3.457 7.189 6.651 9.316 3.954 2.633 9.03 3.889 14.182 3.766.861-.02 1.903-.104 3.117-.251 4.362-.529 8.706-1.906 11.774-4.233 2.345-1.779 3.906-4.257 3.829-7.495a14.603 14.603 0 0 0-.114-1.225c-.505-4.161-2.885-6.947-6.055-8.861-4.122-2.488-9.59-3.814-14.721-4.656-6.604-1.085-13.774-3.059-19.639-6.888-6.812-4.447-11.705-11.027-12.181-20.843-.473-9.748 3.724-17.564 10.344-22.947 4.103-3.336 9.15-5.668 14.537-6.861v-5.034a8.168 8.168 0 0 1 16.336 0v4.914c.484.102.966.213 1.446.335 9.815 2.491 18.743 8.943 23.432 20.111z" clipRule="evenodd"></path>
                  </svg>
                  <h2 className="max-w-[40%] text-xl sm:text-2xl lg:text-3xl font-bold text-white group-hover:text-black mb-4">Cost Efficient</h2>
                  <p className="text-base sm:text-xl text-gray-300 group-hover:text-black">Achieve significant savings compared to in-house staffing without compromising quality.</p>
                </div>
              </div>

              {/* Card 3 */}
              <div className="group relative flex flex-col items-left gap-8 text-left p-6 lg:p-10 bg-[#1f1f1f] rounded-lg shadow-lg w-[240px] xxs:w-[260px] xs:w-[280px] xxsm:w-[330px] xsm:w-[250px] sm:w-[245px] md:w-[310px] lg:w-[300px] flex-shrink-0 overflow-hidden snap-start">
                <div className="absolute inset-0 bg-[#ffaa17] transform origin-bottom scale-y-0 group-hover:scale-y-100 transition-transform duration-700 ease-in-out"></div>
                <div className="relative z-10">
                  <svg fill="currentColor" className="h-16 w-16 group-hover:text-black text-[#ffaa17] mb-6" version="1.0" xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 512.000000 512.000000"
                      preserveAspectRatio="xMidYMid meet">
                      <g transform="translate(0.000000,512.000000) scale(0.100000,-0.100000)"
                      stroke="none">
                      <path d="M1939 4908 c-1 -2 -24 -118 -53 -258 l-52 -255 -114 -41 c-63 -22 -151 -59 -196 -82 l-81 -41 -216 145 c-119 79 -220 144 -224 144 -5 0 -104 -96 -222 -214 l-213 -213 141 -212 142 -212 -60 -117 c-32 -64 -73 -158 -91 -207 l-31 -91 -245 -48 -244 -49 0 -307 0 -307 187 -37 c309 -62 281 -53 289 -89 17 -82 60 -202 111 -308 l55 -116 -127 -193 -126 -193 213 -214 c117 -117 216 -213 221 -213 4 0 88 54 186 119 l179 120 133 -68 c74 -37 181 -82 239 -101 58 -19 107 -35 109 -37 2 -1 23 -96 45 -210 l42 -208 128 -3 c70 -1 126 1 124 5 -98 194 -149 328 -184 493 -25 114 -26 489 -2 592 l15 67 -51 17 c-273 89 -484 311 -573 601 -24 79 -26 104 -26 238 0 175 18 258 83 390 135 272 391 451 698 487 374 44 745 -169 902 -516 17 -36 32 -66 34 -66 3 0 51 11 107 25 344 83 738 42 1038 -109 39 -20 73 -36 76 -36 3 0 5 38 5 85 0 80 -2 86 -22 94 -13 4 -123 28 -245 52 l-222 43 -32 91 c-18 49 -59 143 -91 207 l-59 117 142 212 141 212 -213 214 c-135 134 -220 212 -230 210 -8 -2 -104 -63 -213 -136 -108 -73 -203 -134 -211 -137 -7 -3 -49 12 -92 32 -43 21 -130 57 -193 79 l-114 40 -51 255 -51 255 -307 3 c-169 1 -307 1 -308 0z"/>
                      <path d="M3315 2951 c-273 -49 -518 -175 -720 -372 -56 -54 -127 -134 -158 -178 -119 -164 -207 -372 -243 -571 -23 -127 -23 -353 0 -480 102 -567 541 -1010 1111 -1121 109 -22 418 -19 525 5 429 94 770 357 962 741 55 109 91 214 120 349 30 142 30 388 0 531 -28 131 -54 210 -107 322 -194 412 -567 696 -1013 773 -144 25 -338 25 -477 1z m687 -1028 c10 -9 18 -24 18 -33 0 -19 -233 -708 -250 -740 -16 -30 -46 -43 -70 -30 -47 25 -46 30 85 416 69 203 133 376 142 386 20 22 51 23 75 1z m-1107 -61 c78 -40 115 -97 115 -178 0 -92 -49 -157 -193 -255 -51 -34 -105 -73 -120 -86 l-29 -23 147 0 c142 0 147 -1 170 -25 30 -29 31 -49 5 -75 -19 -19 -33 -20 -220 -20 -187 0 -201 1 -220 20 -41 41 -16 133 52 196 18 17 81 64 141 104 59 39 116 85 127 100 20 28 26 73 14 105 -18 46 -129 59 -235 25 -57 -17 -66 -18 -86 -4 -45 29 -25 83 39 107 111 42 223 46 293 9z m601 4 c17 -13 19 -31 24 -203 l5 -188 35 -3 c40 -4 70 -27 70 -54 0 -30 -36 -58 -75 -58 l-35 0 0 -62 c0 -69 -17 -98 -58 -98 -45 0 -62 26 -62 97 l0 63 -139 0 c-125 0 -141 2 -165 21 -19 15 -26 30 -26 56 0 30 18 59 122 192 205 263 194 251 242 251 23 0 51 -6 62 -14z m1064 -6 c38 -38 27 -71 -123 -370 -141 -282 -142 -285 -175 -288 -42 -4 -62 12 -62 49 0 17 50 129 120 269 l120 240 -160 0 c-147 0 -162 2 -180 20 -11 11 -20 29 -20 40 0 11 9 29 20 40 19 19 33 20 230 20 197 0 211 -1 230 -20z"/>
                      <path d="M3310 1600 c-47 -61 -86 -115 -88 -120 -6 -16 168 -13 174 3 2 7 3 61 2 120 l-3 107 -85 -110z"/>
                      </g>
                  </svg>
                  <h2 className="max-w-[40%] text-xl sm:text-2xl lg:text-3xl font-bold text-white group-hover:text-black mb-4">24/7 Coverage</h2>
                  <p className="text-base sm:text-xl text-gray-300 group-hover:text-black">Round-the-clock support tailored to your shift structures, including weekends and holidays.</p>
                </div>
              </div>

              {/* Card 4 */}
              <div className="group relative flex flex-col items-left gap-8 text-left p-6 lg:p-10 bg-[#1f1f1f] rounded-lg shadow-lg w-[240px] xxs:w-[260px] xs:w-[280px] xxsm:w-[330px] xsm:w-[250px] sm:w-[245px] md:w-[310px] lg:w-[300px] flex-shrink-0 overflow-hidden snap-start">
                <div className="absolute inset-0 bg-[#ffaa17] transform origin-bottom scale-y-0 group-hover:scale-y-100 transition-transform duration-700 ease-in-out"></div>
                <div className="relative z-10">
                <svg fill="currentColor" className="h-16 w-16 group-hover:text-black text-[#ffaa17] mb-6" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" 
                      viewBox="0 0 44.75 44.75" xml:space="preserve">
                      <g>
                        <g id="_x32__137_">
                          <g>
                            <path d="M33.285,19.991c-0.003-0.039-0.004-0.078-0.008-0.116c-0.033-1.48-0.291-2.676-1.455-3.169
                              c-1.23-0.521-2.314-0.528-3.109-0.219c-0.052,0.021-0.188,0.2-0.24,0.233c-0.053,0.033-0.332-0.06-0.437-0.035
                              c-1.745,0.249-2.321,1.893-2.194,3.175c0.417,0.728,0.549,1.646,0.588,2.595c0.65,1.134,1.766,2.005,3.098,2.005
                              c1.779,0,3.188-1.502,3.627-3.137c0.168-0.019,0.307-0.146,0.32-0.32l0.055-0.646C33.544,20.188,33.438,20.043,33.285,19.991z
                              M29.525,23.96c-1.836,0-3.209-1.945-3.209-3.686c0-0.082,0.007-0.162,0.013-0.242c0.765-0.28,2.235-0.865,2.396-1.173
                              c0.668,0.694,3.028,1.084,4.065,1.215c0.005,0.066,0.011,0.133,0.011,0.2C32.799,21.979,31.369,23.96,29.525,23.96z"/>
                            <path d="M9.641,20.342l0.055,0.646c0.016,0.182,0.166,0.313,0.344,0.323c0.429,1.639,1.814,3.148,3.569,3.148
                              c1.377,0,2.525-0.902,3.181-2.065c-0.014-0.813,0.2-1.648,0.664-2.339c-0.029-0.021-0.05-0.052-0.085-0.064
                              c-0.003-0.039-0.004-0.078-0.008-0.116c-0.033-1.48-0.291-2.676-1.455-3.169c-1.23-0.521-2.314-0.528-3.109-0.219
                              c-0.052,0.021-0.187,0.2-0.24,0.233c-0.053,0.033-0.332-0.06-0.437-0.035c-1.745,0.249-2.321,1.894-2.194,3.176
                              c-0.004,0.036-0.006,0.072-0.009,0.108C9.746,20.01,9.625,20.165,9.641,20.342z M10.411,20.033
                              c0.765-0.28,2.235-0.865,2.395-1.173c0.668,0.694,3.029,1.084,4.066,1.215c0.005,0.066,0.011,0.133,0.011,0.2
                              c0,1.704-1.43,3.686-3.273,3.686c-1.837,0-3.209-1.945-3.209-3.686C10.399,20.192,10.405,20.112,10.411,20.033z"/>
                            <path d="M19.021,8.686c0.261,0.997,1.103,1.915,2.171,1.915c1.083,0,1.938-0.914,2.207-1.908
                              c0.103-0.012,0.187-0.089,0.195-0.195l0.033-0.393c0.01-0.103-0.057-0.191-0.148-0.223c-0.002-0.023-0.002-0.047-0.004-0.07
                              c-0.021-0.9-0.178-1.627-0.886-1.928c-0.749-0.317-1.408-0.322-1.892-0.133c-0.032,0.013-0.114,0.122-0.146,0.142
                              c-0.032,0.02-0.202-0.036-0.266-0.021c-1.061,0.151-1.411,1.151-1.334,1.932c-0.003,0.021-0.004,0.044-0.006,0.066
                              c-0.103,0.025-0.176,0.119-0.167,0.227l0.034,0.393C18.821,8.601,18.913,8.681,19.021,8.686z M19.247,7.908
                              c0.465-0.17,1.359-0.526,1.457-0.714c0.406,0.422,1.842,0.66,2.473,0.739c0.003,0.041,0.007,0.081,0.007,0.122
                              c0,1.036-0.871,2.241-1.992,2.241c-1.118,0-1.952-1.183-1.952-2.241C19.24,8.005,19.243,7.957,19.247,7.908z"/>
                            <path d="M14.169,8.614c-1.474,0.21-1.96,1.599-1.853,2.683c-0.003,0.03-0.005,0.061-0.008,0.091
                              c-0.143,0.035-0.245,0.165-0.231,0.315l0.047,0.546c0.013,0.154,0.14,0.265,0.29,0.272c0.363,1.384,1.532,2.659,3.015,2.659
                              c1.503,0,2.692-1.269,3.064-2.649c0.141-0.016,0.258-0.124,0.271-0.271l0.046-0.545c0.012-0.143-0.078-0.265-0.206-0.31
                              c-0.003-0.032-0.003-0.065-0.007-0.098c-0.028-1.25-0.246-2.26-1.229-2.677c-1.039-0.44-1.955-0.446-2.626-0.185
                              c-0.044,0.018-0.158,0.169-0.203,0.197C14.494,8.67,14.259,8.592,14.169,8.614z M14.75,10.451
                              c0.564,0.586,2.558,0.916,3.434,1.026c0.004,0.056,0.009,0.112,0.009,0.169c0,1.439-1.208,3.112-2.765,3.112
                              c-1.552,0-2.71-1.643-2.71-3.112c0-0.069,0.005-0.137,0.01-0.204C13.374,11.206,14.615,10.71,14.75,10.451z"/>
                            <path d="M23.964,12.521c0.363,1.384,1.532,2.659,3.015,2.659c1.504,0,2.692-1.269,3.064-2.649
                              c0.098-0.011,0.168-0.075,0.216-0.156l0.074-0.008c0.017-0.002,0.047-0.013,0.05-0.043l0.047-0.544
                              c0.002-0.037-0.035-0.051-0.039-0.052l-0.031-0.011c0.012-0.143-0.078-0.265-0.207-0.31c-0.002-0.032-0.002-0.065-0.006-0.098
                              c-0.028-1.25-0.246-2.26-1.229-2.677c-1.039-0.44-1.955-0.446-2.626-0.185c-0.044,0.018-0.158,0.169-0.203,0.197
                              c-0.044,0.027-0.279-0.05-0.369-0.029c-1.474,0.21-1.96,1.599-1.853,2.683c-0.003,0.03-0.005,0.061-0.009,0.091
                              c-0.143,0.035-0.244,0.165-0.23,0.315l0.047,0.546C23.688,12.402,23.814,12.515,23.964,12.521z M24.277,11.441
                              c0.646-0.236,1.888-0.731,2.022-0.991c0.563,0.586,2.558,0.916,3.435,1.026c0.004,0.056,0.009,0.112,0.009,0.169
                              c0,1.439-1.208,3.112-2.765,3.112c-1.553,0-2.711-1.643-2.711-3.112C24.27,11.577,24.273,11.508,24.277,11.441z"/>
                            <path d="M18.695,12.731c-0.173,0.561-0.49,1.111-0.907,1.572l3.395,0.002v-0.006l3.467-0.002
                              c-0.407-0.459-0.717-1.008-0.887-1.567c-0.187-0.076-0.32-0.249-0.338-0.459l-0.047-0.546c-0.018-0.201,0.076-0.388,0.236-0.498
                              c-0.018-0.192-0.01-0.39,0.011-0.588c-0.202-0.208-0.438-0.355-0.723-0.396c-0.056-0.009-0.111,0.004-0.147,0.019l-0.894,1.209
                              l-0.679-0.541l-0.004,0.003v0.003l-0.001-0.001L20.5,11.477l-0.894-1.209c-0.036-0.014-0.091-0.027-0.146-0.018
                              c-0.256,0.036-0.471,0.161-0.66,0.335c0.024,0.223,0.04,0.448,0.046,0.671c0.146,0.112,0.229,0.292,0.213,0.481l-0.046,0.544
                              C18.995,12.483,18.872,12.65,18.695,12.731z"/>
                            <path d="M33.085,21.764c-0.262,0.669-0.694,1.294-1.246,1.756C32.398,23.052,32.83,22.424,33.085,21.764z"/>
                            <path d="M12.402,16.458c0.019,0.003,0.037,0.007,0.054,0.01c0.007-0.008,0.014-0.016,0.022-0.025
                              c0.089-0.098,0.147-0.159,0.226-0.19c0.45-0.175,0.959-0.242,1.5-0.212l-0.978-1.323c-0.05-0.02-0.126-0.037-0.203-0.025
                              c-0.917,0.129-1.495,1.018-1.862,2.051c0.271-0.142,0.569-0.257,0.922-0.308l0.081-0.007
                              C12.23,16.431,12.319,16.444,12.402,16.458z"/>
                            <path d="M16.695,23.382l0.068,0.795c0.019,0.225,0.204,0.388,0.423,0.398c0.528,2.016,2.232,3.875,4.392,3.875
                              c2.191,0,3.923-1.85,4.464-3.861c0.206-0.023,0.376-0.18,0.396-0.395l0.067-0.794c0.019-0.207-0.112-0.386-0.301-0.45
                              c-0.004-0.047-0.005-0.096-0.011-0.143c-0.039-1.821-0.356-3.293-1.791-3.9c-1.513-0.641-2.847-0.65-3.825-0.27
                              c-0.064,0.025-0.23,0.246-0.295,0.287c-0.065,0.04-0.408-0.073-0.538-0.042c-2.147,0.306-2.856,2.33-2.7,3.908
                              c-0.004,0.044-0.008,0.088-0.011,0.133C16.824,22.974,16.675,23.163,16.695,23.382z M17.643,23.001
                              c0.94-0.344,2.75-1.064,2.946-1.443c0.823,0.854,3.728,1.334,5.003,1.494c0.006,0.082,0.014,0.164,0.014,0.246
                              c0,2.098-1.76,4.535-4.028,4.535c-2.261,0-3.949-2.394-3.949-4.535C17.628,23.198,17.636,23.1,17.643,23.001z"/>
                            <path d="M16.359,16.39l-0.942-0.751l-0.006,0.005v0.005l-0.001-0.001l-0.604,0.481c0.392,0.073,0.791,0.176,1.196,0.348
                              c1.344,0.569,1.57,1.998,1.605,3.335c0.476-0.594,1.158-1.043,2.1-1.178l0.091-0.008c0.079,0,0.185,0.017,0.284,0.035
                              c0.018,0.003,0.034,0.006,0.051,0.009c-0.245-1.585-0.849-3.776-2.331-3.985c-0.078-0.012-0.154,0.005-0.204,0.026L16.359,16.39z
                              "/>
                            <path d="M29.354,14.685c-0.078-0.012-0.153,0.005-0.203,0.026l-1.241,1.68l-0.942-0.751l-0.006,0.005v0.005l-0.001-0.001
                              l-0.942,0.751l-1.242-1.68c-0.049-0.02-0.125-0.037-0.202-0.025c-1.321,0.186-1.946,1.949-2.239,3.453
                              c0.691,0.042,1.425,0.217,2.166,0.53c0.463,0.196,0.812,0.474,1.083,0.808c-0.009-1.34,0.704-2.802,2.417-3.047l0.081-0.007
                              c0.065,0,0.155,0.014,0.239,0.029c0.019,0.003,0.037,0.007,0.054,0.01c0.007-0.008,0.015-0.016,0.022-0.025
                              c0.088-0.098,0.146-0.159,0.226-0.19c0.683-0.266,1.503-0.29,2.364-0.083C30.619,15.395,30.098,14.79,29.354,14.685z"/>
                            <path d="M21.559,35.933l7.115-0.006c0,0-0.217-7.719-3.635-8.2c-0.113-0.019-0.225,0.008-0.296,0.037l-1.809,2.446l-1.373-1.094
                              l-0.008,0.006v0.008l-0.002-0.002l-1.373,1.095l-1.809-2.448c-0.072-0.029-0.183-0.054-0.296-0.037
                              c-3.418,0.48-3.635,8.201-3.635,8.201l7.12,0.004L21.559,35.933L21.559,35.933z"/>
                            <path d="M29.51,30.542l5.782-0.004c0,0-0.176-6.273-2.954-6.666c-0.092-0.014-0.182,0.006-0.24,0.03l-1.471,1.989l-1.115-0.891
                              l-0.007,0.006v0.007l-0.001-0.002l-1.115,0.89l-1.47-1.988c-0.05-0.021-0.128-0.029-0.205-0.025l-0.028,0.328
                              c-0.021,0.268-0.195,0.488-0.441,0.58c-0.352,1.178-1.133,2.338-2.197,3.086c-0.037,0.183-0.068,0.361-0.098,0.535l0.635-0.858
                              l0.064-0.026c0.072-0.029,0.244-0.082,0.429-0.053c1.424,0.201,2.317,1.527,2.883,3.07l1.55,0.002V30.542z"/>
                            <path d="M7.807,30.548l5.786,0.002v-0.01h1.567c0.567-1.535,1.459-2.851,2.878-3.049c0.168-0.025,0.352,0.022,0.423,0.051
                              l0.066,0.026l0.622,0.841c-0.033-0.201-0.07-0.406-0.114-0.617c-0.989-0.75-1.717-1.865-2.05-3.002
                              c-0.26-0.084-0.448-0.312-0.472-0.592l-0.026-0.308c-0.023-0.004-0.042-0.015-0.066-0.019c-0.092-0.015-0.182,0.006-0.241,0.029
                              l-1.47,1.989l-1.115-0.89l-0.007,0.006v0.006l-0.001-0.002l-1.115,0.89l-1.47-1.989c-0.059-0.022-0.149-0.043-0.241-0.029
                              C7.983,24.273,7.807,30.548,7.807,30.548z"/>
                            <path d="M37.445,4.883l2.582-3.898l-9.471,1.197l2.92,8.694l2.287-3.452c3.682,3.526,5.986,8.475,5.986,13.964
                              c0,10.685-8.691,19.375-19.375,19.375C11.691,40.764,3,32.073,3,21.389C3,13.463,7.787,6.645,14.617,3.646L12.36,1.39
                              C5.039,5.072,0,12.65,0,21.39c0,12.337,10.038,22.375,22.375,22.375S44.75,33.727,44.75,21.39
                              C44.75,14.855,41.927,8.978,37.445,4.883z"/>
                          </g>
                        </g>
                      </g>
                  </svg>
                  <h2 className="max-w-[40%] text-xl sm:text-2xl lg:text-3xl font-bold text-white group-hover:text-black mb-4">Trained Staff</h2>
                  <p className="text-base sm:text-xl text-gray-300 group-hover:text-black">All personnel are trained in security protocols and communication etiquette specific to dispatch and monitoring.</p>
                </div>
              </div>
              
              {/* Card 5 */}
              <div className="group relative flex flex-col items-left gap-8 text-left p-6 lg:p-10 bg-[#1f1f1f] rounded-lg shadow-lg w-[240px] xxs:w-[260px] xs:w-[280px] xxsm:w-[330px] xsm:w-[250px] sm:w-[245px] md:w-[310px] lg:w-[300px] flex-shrink-0 overflow-hidden snap-start">
                <div className="absolute inset-0 bg-[#ffaa17] transform origin-bottom scale-y-0 group-hover:scale-y-100 transition-transform duration-700 ease-in-out"></div>
                <div className="relative z-10">
                <svg fill="currentColor" className="h-16 w-16 group-hover:text-black text-[#ffaa17] mb-6" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" 
                      viewBox="0 0 399.997 399.997"
                      xml:space="preserve">
                      <g>
                        <g>
                          <g>
                            <path d="M366.684,24.971H33.316C14.945,24.971,0,39.917,0,58.288V275.56c0,18.371,14.945,33.317,33.316,33.317h107.412
                              l-20.387,44.955c-2.105,4.643-1.709,10.037,1.053,14.321c2.762,4.285,7.51,6.873,12.607,6.873h131.996c0,0,0.014,0,0.02,0
                              c8.285,0,15-6.715,15-15c0-2.493-0.605-4.848-1.686-6.916l-20.062-44.233h107.412c18.371,0,33.316-14.946,33.316-33.317V58.288
                              C400,39.917,385.055,24.971,366.684,24.971z M370,275.561c0,1.829-1.488,3.316-3.316,3.316H33.316
                              c-1.828,0-3.316-1.487-3.316-3.316V58.288c0-1.829,1.488-3.317,3.316-3.317h333.367c1.828,0,3.316,1.488,3.316,3.317L370,275.561
                              L370,275.561z"/>
                            <path d="M270.717,102.373c-0.875-0.532-21.94-13.063-70.717-13.063c-48.774,0-69.844,12.531-70.716,13.063
                              c-2.779,1.697-4.415,4.771-4.276,8.022c0.041,0.958,1.11,23.836,10.078,52.987c5.285,17.18,12.238,33.006,20.669,47.054
                              c10.647,17.738,23.686,32.667,38.757,44.371c1.613,1.255,3.549,1.883,5.488,1.883c1.939,0,3.875-0.628,5.488-1.883
                              c15.069-11.704,28.108-26.633,38.756-44.371c8.43-14.048,15.385-29.874,20.67-47.054c8.969-29.151,10.038-52.029,10.078-52.987
                              C275.131,107.144,273.494,104.07,270.717,102.373z M233.23,145.784l-39.563,39.557c-1.143,1.152-2.665,1.781-4.279,1.781
                              c-1.619,0-3.141-0.629-4.291-1.781l-18.328-18.363c-2.359-2.362-2.359-6.2,0-8.563l3.529-3.53
                              c1.148-1.136,2.671-1.765,4.287-1.765c1.616,0,3.131,0.623,4.271,1.765l10.528,10.493l31.72-31.719
                              c1.146-1.149,2.663-1.781,4.284-1.781c1.618,0,3.14,0.632,4.277,1.781l3.563,3.561C235.59,139.581,235.59,143.422,233.23,145.784
                              z"/>
                          </g>
                        </g>
                      </g>
                  </svg>
                  <h2 className="max-w-[40%] text-xl sm:text-2xl lg:text-3xl font-bold text-white group-hover:text-black mb-4">Data Security</h2>
                  <p className="text-base sm:text-xl text-gray-300 group-hover:text-black">We take your confidentiality seriously, with secure systems and strict access controls.</p>
                </div>
              </div>

              {/* Card 6 */}
              <div className="group relative flex flex-col items-left gap-8 text-left p-6 lg:p-10 bg-[#1f1f1f] rounded-lg shadow-lg w-[240px] xxs:w-[260px] xs:w-[280px] xxsm:w-[330px] xsm:w-[250px] sm:w-[245px] md:w-[310px] lg:w-[300px] flex-shrink-0 overflow-hidden snap-start">
                <div className="absolute inset-0 bg-[#ffaa17] transform origin-bottom scale-y-0 group-hover:scale-y-100 transition-transform duration-700 ease-in-out"></div>
                <div className="relative z-10">
                <svg fill="currentColor" className="h-16 w-16 group-hover:text-black text-[#ffaa17] mb-6" version="1.1" id="_x32_" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" 
                      viewBox="0 0 512 512"  xml:space="preserve">
                      <g>
                        <path class="st0" d="M435.95,287.525c32.51,0,58.87-26.343,58.87-58.853c0-32.51-26.361-58.871-58.87-58.871
                          c-32.502,0-58.863,26.361-58.863,58.871C377.088,261.182,403.448,287.525,435.95,287.525z"/>
                        <path class="st0" d="M511.327,344.251c-2.623-15.762-15.652-37.822-25.514-47.677c-1.299-1.306-7.105-1.608-8.673-0.636
                          c-11.99,7.374-26.074,11.714-41.19,11.714c-15.099,0-29.184-4.34-41.175-11.714c-1.575-0.972-7.373-0.67-8.672,0.636
                          c-2.757,2.757-5.765,6.427-8.698,10.683c7.935,14.94,14.228,30.81,16.499,44.476c2.27,13.7,1.533,26.67-2.138,38.494
                          c13.038,4.717,28.673,6.787,44.183,6.787C476.404,397.014,517.804,382.987,511.327,344.251z"/>
                        <path class="st0" d="M254.487,262.691c52.687,0,95.403-42.716,95.403-95.402c0-52.67-42.716-95.386-95.403-95.386
                          c-52.678,0-95.378,42.716-95.378,95.386C159.109,219.975,201.808,262.691,254.487,262.691z"/>
                        <path class="st0" d="M335.269,277.303c-2.07-2.061-11.471-2.588-14.027-1.006c-19.448,11.966-42.271,18.971-66.755,18.971
                          c-24.466,0-47.3-7.005-66.738-18.971c-2.555-1.583-11.956-1.055-14.026,1.006c-16.021,16.004-37.136,51.782-41.384,77.288
                          c-10.474,62.826,56.634,85.508,122.148,85.508c65.532,0,132.639-22.682,122.165-85.508
                          C372.404,329.085,351.289,293.307,335.269,277.303z"/>
                        <path class="st0" d="M76.049,287.525c32.502,0,58.862-26.343,58.862-58.853c0-32.51-26.36-58.871-58.862-58.871
                          c-32.511,0-58.871,26.361-58.871,58.871C17.178,261.182,43.538,287.525,76.049,287.525z"/>
                        <path class="st0" d="M115.094,351.733c2.414-14.353,9.225-31.253,17.764-46.88c-2.38-3.251-4.759-6.083-6.955-8.279
                          c-1.299-1.306-7.097-1.608-8.672-0.636c-11.991,7.374-26.076,11.714-41.182,11.714c-15.108,0-29.202-4.34-41.183-11.714
                          c-1.568-0.972-7.382-0.67-8.681,0.636c-9.887,9.854-22.882,31.915-25.514,47.677c-6.468,38.736,34.924,52.762,75.378,52.762
                          c14.437,0,29.016-1.777,41.459-5.84C113.587,379.108,112.757,365.835,115.094,351.733z"/>
                      </g>
                  </svg>
                  <h2 className="max-w-[40%] text-xl sm:text-2xl lg:text-3xl font-bold text-white group-hover:text-black mb-4">Scalable Teams</h2>
                  <p className="text-base sm:text-xl text-gray-300 group-hover:text-black">Easily scale up or down based on your needs — without the hassle of hiring and training.</p>
                </div>
              </div>
            </div>
            
        </div>
        <div className="w-full max-w-[122rem] mx-auto mt-36 xxs:mt-30 xxsm:mt-28 xsm:mt-28 sm:mt-32 md:mt-30 lg:mt-34 xl:mt-30 xxl:mt-32 absolute -inset-0 top-1/2  -translate-y-1/2 flex justify-between px-[0rem] sm:px-[0rem] lg:px-[-3rem] xl:px-[-3rem]">
                        <div className=''>
                        <button 
                            onClick={() => scroll(-1)} 
                            disabled={!canScrollLeft}
                            className="bg-white/50 hover:bg-[#ffaa17] disabled:opacity-50 disabled:cursor-not-allowed text-black rounded-full p-2"
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
                            </svg>
                        </button>
                        </div>
                        <div className=''>
                        <button 
                            onClick={() => scroll(1)} 
                            disabled={!canScrollRight}
                            className="bg-white/50 hover:bg-[#ffaa17] disabled:opacity-50 disabled:cursor-not-allowed text-black rounded-full p-2"
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                            </svg>
                        </button>
                        </div>
        </div>
      </div>
    </div>
  );
};

export default WhyChooseUs;